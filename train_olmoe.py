...
def train(model, layer, hook_name):
    cfg = LanguageModelSAERunnerConfig(
        model_name=MODEL_NAME,
        hook_name=f"blocks.{layer}.{hook_name}",
        hook_layer=layer,
        ...
        # WANDB
        log_to_wandb=True,
        wandb_project=f"SAE-olmoe-1b-7b-0924-layer{layer}-{hook_name}",
        wandb_log_frequency=30,
        eval_every_n_wandb_logs=20,
        # Misc
        device=device,
        n_checkpoints=10,
        checkpoint_path=f"checkpoints-layer{layer}-{hook_name}",
        dtype="bfloat16",
    )
...
if __name__ == "__main__":
    model = HookedTransformer.from_pretrained(MODEL_NAME, first_n_layers=11)
    for layer in [6, 7, 8, 9, 10]:
        for hook_name in ["hook_resid_mid", "hook_resid_post"]:
            train(model, layer=layer, hook_name=hook_name)
